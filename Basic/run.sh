...
iverilog '-Wall' design.sv  && unbuffer vvp -M $COCOTB_LIB -m gpivpi a.out  ; echo 'Creating result.zip...' && zip -r /tmp/tmp_zip_file_123play.zip . && mv /tmp/tmp_zip_file_123play.zip result.zip